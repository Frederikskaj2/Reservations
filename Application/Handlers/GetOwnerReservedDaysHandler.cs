...
public static class GetOwnerReservedDaysHandler
{
    public static EitherAsync<Failure, IEnumerable<MyReservedDay>> Handle(IPersistenceContextFactory contextFactory, ReservedDaysCommand command) =>
        ...
        select orders.Bind(
            order => order.Reservations
                ...
                .Bind(reservation => CreateMyReservedDays(order.OrderId, reservation, order.Flags.HasFlag(OrderFlags.IsOwnerOrder))));
}
