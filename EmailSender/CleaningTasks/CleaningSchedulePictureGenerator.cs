...
static class CleaningSchedulePictureGenerator
{
    ...
    static Image CreateMonthImage(Table table)
    {
    ...
    static Image CreateLegendImage(IReadOnlyList<string> orderedResourceNames)
    {
        ...
        var verticalMargin = (height - boxHeight)/2;
    ...
    static async ValueTask<Picture> CreatePicture(Image image, CancellationToken cancellationToken)
    {
        ...
        return new Picture(image.Width, image.Height, "image/png", stream.ToArray().AsImmutableArrayUnsafe());
    }
