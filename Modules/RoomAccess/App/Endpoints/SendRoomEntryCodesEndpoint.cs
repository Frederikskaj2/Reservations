...
class SendRoomEntryCodesEndpoint
{
    SendRoomEntryCodesEndpoint() { }

    [Authorize(Roles = nameof(Roles.Jobs))]
    public static Task<IResult> Handle(
        [FromServices] IDateProvider dateProvider,
        [FromServices] IRoomAccessEmailService emailService,
        [FromServices] IEntityReader entityReader,
        [FromServices] IEntityWriter entityWriter,
        [FromServices] ILogger<SendRoomEntryCodesEndpoint> logger,
        [FromServices] IOptionsSnapshot<OrderingOptions> options,
        HttpContext httpContext)
    {
        var either = SendRoomEntryCodes(emailService, options.Value, entityReader, entityWriter, new(dateProvider.Today), httpContext.RequestAborted);
