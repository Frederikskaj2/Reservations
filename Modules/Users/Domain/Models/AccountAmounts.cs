...
public sealed class AccountAmounts : IEnumerable<(Account Account, Amount Amount)>
{
    ...
    public AccountAmounts ApplyReverse(AccountAmounts amounts) =>
        ValidateAmounts(amounts.hashMap.Fold(this, folder: (accountAmounts, tuple) => accountAmounts.ApplyAmount((tuple.Key, Amount.Negate(tuple.Value)))));
    ...
    IEnumerator<(Account Key, Amount Value)> GetEnumeratorForNonZeroAmounts() =>
        ...
        hashMap.Filter(tuple => tuple.Value != Amount.Zero).GetEnumerator();
}
