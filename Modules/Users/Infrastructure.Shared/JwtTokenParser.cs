...
public static class JwtTokenParser
{
    ...
    static readonly Dictionary<char, byte> lookupTable = characterSet.ToCharArray()
        .Select((c, i) => (Character: c, Index: i)).ToDictionary(tuple => tuple.Character, tuple => (byte) tuple.Index);

    public static IReadOnlyDictionary<string, JsonElement>? Parse(ReadOnlySpan<char> token)
    {
