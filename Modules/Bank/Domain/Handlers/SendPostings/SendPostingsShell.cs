...
public static class SendPostingsShell
{
    public static EitherAsync<Failure<Unit>, EmailAddress> SendPostings(
        ...
        from postings in GetPostingsV1OrV2(reader, command.FromMonth, command.ToMonth, cancellationToken).MapReadError()
        ...
        from _ in emailService
            .Send(new(user.Email(), user.FullName, command.FromMonth, command.ToMonth, postings), users, cancellationToken)
