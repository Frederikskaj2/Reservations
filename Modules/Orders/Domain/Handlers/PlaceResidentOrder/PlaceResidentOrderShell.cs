...
public static class PlaceResidentOrderShell
{
    public static EitherAsync<Failure<Unit>, ResidentOrder> PlaceResidentOrder(
        ...
        from administratorEntity in reader.ReadWithETag<User>(command.AdministratorId, cancellationToken).MapReadError()
    ...
    static EitherAsync<Failure<Unit>, Seq<(EntityOperation Operation, ETag ETag)>> Write(
        ...
        ETaggedEntity<User> administratorEntity,
        PlaceResidentOrderOutput output,
        CancellationToken cancellationToken) =>
        writer
            .Write(
                collector => collector.Add(userEntity).Add(administratorEntity),
                tracker => tracker.Update(output.User).Add(output.Order).Add(output.Transaction).Update(output.Administrator),
                cancellationToken)
            .MapWriteError();
