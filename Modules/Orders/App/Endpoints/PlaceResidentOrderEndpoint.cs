...
using static Frederikskaj2.Reservations.Orders.PlaceResidentOrderShell;
using static Frederikskaj2.Reservations.Orders.Validator;

namespace Frederikskaj2.Reservations.Orders;

class PlaceResidentOrderEndpoint
{
    PlaceResidentOrderEndpoint() { }

    [Authorize(Roles = nameof(Roles.OrderHandling))]
    public static Task<IResult> Handle(
        [FromBody] PlaceResidentOrderRequest request,
        ...
        [FromServices] ILogger<PlaceResidentOrderEndpoint> logger,
        ...
        HttpContext httpContext)
    {
        ...
        var either =
            ...
            from command in ValidatePlaceResidentOrder(now, request, userId).ToAsync()
            from order in PlaceResidentOrder(
                emailService,
            ...
            select new PlaceResidentOrderResponse(CreateMyOrder(order));
