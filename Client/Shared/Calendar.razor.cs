...
public sealed partial class Calendar : IDisposable
{
    ...
    [Parameter] public IEnumerable<MyReservedDay>? ReservedDays { get; set; }
    ...
    {
        if (IsResourceAvailable is null)
            throw new ArgumentNullException(nameof(IsResourceAvailable));
        if (ReservedDays is null)
            throw new ArgumentNullException(nameof(ReservedDays));
        ...
        {
            var response = await ApiClient.GetAsync<IEnumerable<OrderSummary>>("/orders/any");
            if (response.IsSuccess)
            {
                var orders = response.Result!;
                tooltips = orders.ToDictionary(order => order.OrderId, order => $"Bestilling {order.OrderId}: {order.UserInformation.FullName}");
            }
            else
                tooltips = new();
        }
        ...
        var days = Enumerable.Range(0, dayCount)
            ...
            .Select(date =>
                new Day(date,
                    ...
                    HighPricePolicy.IsHighPriceDay(date, DataProvider.Holidays),
    ...
    Task MakeReservationAsync(LocalDate date) => OnMakeReservation.InvokeAsync((currentResourceId, date));

    Task KeyUpAsync(KeyboardEventArgs e, LocalDate date) =>
    ...
    void NavigateToOrder(OrderId orderId) =>
        NavigationManager.NavigateTo($"{Urls.Orders}/{orderId}");

    string? GetTooltip(OrderId orderId) =>
        tooltips!.TryGetValue(orderId, out var tooltip) ? tooltip : null;
